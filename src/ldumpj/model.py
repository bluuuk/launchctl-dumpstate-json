...
class EndPoint(BaseModel):
    port: str = Field(description="The port number of the endpoint in hex")
    ...
    @classmethod
    def port_to_hex(cls, data: Any) -> Any:
        if "port" in data and isinstance(data["port"], int):
            data["port"] = hex(data["port"])[2:]
        return data
...
class LauchctlService(BaseModel):
    ...
    abandon_process_group: Optional[bool] = Field(False, alias='AbandonProcessGroup', description="Whether to abandon the process group.")
    low_priority_io: Optional[bool] = Field(False, alias='LowPriorityIO', description="Whether to use low priority I/O.")
    launch_only_once: Optional[bool] = Field(False, alias='LaunchOnlyOnce', description="Whether to launch the job only once.")
    ...
    # @model_validator(mode="before")
    # @classmethod
    # def _convert_booleans(cls, data: Any) -> Any:
    #     if isinstance(data, dict):
    #         bool_fields = [
    #             "disabled", "RunAtLoad", "LowPriorityIO", "LaunchOnlyOnce",
    #             "AbandonProcessGroup", "JoinExistingSession", "started suspended",
    #             "proxy started suspended", "trampolined", "initialized"
    #         ]
    #         for field in bool_fields:
    #             if field in data and data[field] in [0, 1]:
    ...
    def _normalize_properties(cls, data: Any) -> Any:
        if isinstance(data, dict):
            if "properties" in data and data["properties"] is not None:
                data["properties"] = [prop.strip() for prop in data["properties"].split("|")]
            else:
                data["properties"] = []
        return data
    ...
    def _set_derived_fields(self) -> Self:
        ...
        if not self.program and self.arguments:
